--[[
HTTPClient wrapper
based on script of @author ikubicki
]]
class 'HTTPClient'
...
function HTTPClient:getRedirect(url, success, error, headers)
    local client = net.HTTPClient({timeout = 10000})
    if not headers then
        headers = {}
    end
    client:request(self:url(url), self:requestOptions(success, error, 'GETREDIRECT', nil, headers)) 
end
...
--    QuickApp:debug(self.options.baseUrl .. tostring(url))
